...
`cordova-background-geolocation-lt` is published by GitHub Actions
...
`@transistorsoft/background-geolocation-types` and the Capacitor plugin release the same way.

## A release

```bash
npm run release -- prepare 5.4.2   # branch chore/release-5.4.2: version (package.json and plugin.xml), dated CHANGELOG, preflight
# merge chore/release-5.4.2 into master
npm run release -- tag 5.4.2       # on master: tag the merge (lightweight, like every earlier tag)
...
since the version on npm, a strict consumer compile, a CHANGELOG heading for the version),
then publishes. What ships is decided by `.npmignore`: 5.4.0 shipped `.github/` and a local
Android build's `src/android/.gradle/` caches, which it now excludes and preflight refuses. A prerelease such as `5.5.0-beta.1` is
...
Publishing is only the npm half. The native SDK plugin.xml requires (`TSLocationManager`)
must already be released, or a build fails to resolve it.
...
organization `transistorsoft`, repository `cordova-background-geolocation-lt`, workflow
